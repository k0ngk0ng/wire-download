...
- 支持在独立 Chromium 浏览器中登录并保存网站 Cookie，通过 `login --remote user@server` 可向无桌面的 Linux 传入会话。此可选功能需要桌面端已有 Chromium 浏览器及 SSH 客户端；不自动提取 localStorage/Bearer token。
